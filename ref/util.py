...
class DetectionThread(QThread):
    ...
    def __init__(
            ...
            color_box=(0, 255, 0),
            color_text=(0, 255, 0),
            mask_points=None,           # <--- 추가됨: 마스킹 좌표 리스트
            color_mask=(0, 0, 255)      # <--- 추가됨: 마스킹 라인 색상
        ):
        super().__init__()
        ...
        if mask_points:
            # cv2.polylines 등에 사용하기 위해 numpy 배열로 변환
            self.mask_points = np.array(mask_points, np.int32)
            self.mask_points = self.mask_points.reshape((-1, 1, 2))
    ...
    def run(self):
        ...
        try:
            ...
            if not os.path.exists(self.calib_path):
                self.log_signal.emit("Error: Calibration file not found")
                return
            ...
            self.log_signal.emit(f"Resolution {half_width}x{raw_height}")
            ...
            while self._run_flag and cap.isOpened():
                try:
                    ...
                    if self.mask_points is not None:
                        overay = imgL_rect.copy()
                        cv2.fillPoly(overay, [self.mask_points], self.color_mask)
                        alpha = 0.3
                        cv2.addWeighted(overay, alpha, imgL_rect, 1 - alpha, 0, imgL_rect)
                    ...
                    for box in boxes:
                        ...
                        # 제외영역
                        if self.mask_points is not None:
                            dist_from_poly = cv2.pointPolygonTest(self.mask_points, (center_x_pt, center_y_pt), False)
                            # dist > 0 : 내부, dist < 0 : 외부, dist = 0 : 경계
                            if dist_from_poly >= 0:
                                continue
                        ...
                        d_x1 = int((center_x_pt - crop_w // 2) * disp_scale_x)
                        d_y1 = int((center_y_pt - crop_h // 2) * disp_scale_y)
                        d_x2 = int((center_x_pt + crop_w // 2) * disp_scale_x)
                        d_y2 = int((center_y_pt + crop_h // 2) * disp_scale_y)
                        ...
                        # 중심점
                        cv2.circle(imgL_rect, (center_x_pt, center_y_pt), 3, (0, 0, 255), -1)
                    ...
                    if log_distances:
                        ...
                        if self.last_log_time is None or (current_time - self.last_log_time) > self.log_deduplication_interval:
                            ...
                            full_msg = f"[{time_str}] Person Detected outside ROI: {dist_msg}"
            ...
            if rknn_lite is not None:
                rknn_lite.release()
            self.log_signal.emit("Thread finished")
